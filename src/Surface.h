...
{
public:
    Surface(const std::shared_ptr<VulkanManager>& vulkanManager, const Window& window);
    ~Surface();

    [[nodiscard]] inline VkFormat Format() const { return mFormat; }
    [[nodiscard]] inline std::vector<VkImageView> ImageViews() const { return mSwapchainImageViews; }
    [[nodiscard]] inline VkExtent2D Extent() const { return mExtent; }
    [[nodiscard]] inline uint32_t ImageCount() const { return static_cast<uint32_t>(mSwapchainImages.size()); }
