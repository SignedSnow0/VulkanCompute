...
int main(int argc, char** argv)
{
    ...
    auto vertex = std::make_shared<Shader>(vulkanManager, "assets/shaders/default.vert.glsl", ShaderStage::Vertex, surface->ImageCount());
    auto fragment = std::make_shared<Shader>(vulkanManager, "assets/shaders/default.frag.glsl", ShaderStage::Fragment, surface->ImageCount());
    ...
    auto computeShader = std::make_shared<Shader>(vulkanManager, "assets/shaders/RayTracer.comp.glsl", ShaderStage::Compute, surface->ImageCount());
    auto computePipeline = std::make_shared<ComputePipeline>(vulkanManager, computeShader);
    ...
    while (!window.shouldClose()) {
        ...
        computePipeline->Dispatch(commandBuffer, (surface->Extent().width + 15) / 16, (surface->Extent().height + 15) / 16, 1);
