...
void createBuffer(const std::shared_ptr<VulkanManager>& vulkanManager, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
    ...
    vkGetBufferMemoryRequirements(vulkanManager->Device(), buffer, &memRequirements);

    VkMemoryAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocInfo.allocationSize = memRequirements.size;
    allocInfo.memoryTypeIndex = findMemoryType(vulkanManager->PhysicalDevice(), memRequirements.memoryTypeBits, properties);

    if (vkAllocateMemory(vulkanManager->Device(), &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS)
    {
        return;
    }
...
{
    vulkanManager->SubmitCommnand([&](VkCommandBuffer commandBuffer) {
